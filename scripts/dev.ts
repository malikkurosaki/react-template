...
function scheduleGenerate() {
    ...
    generatePromise = new Promise((resolve) => setTimeout(resolve, 100)).then(async () => {
        await runGenerator();
...
try {
    const watcher = watch(PAGES_DIR, { recursive: true }, async (event, filename) => {
        if (!filename) return;
        ...
        try {
            ...
            if (exists) {
                ...
                if (size === 0) {
                    console.log(`✨ New file detected: ${filename}. Generating template...`);
